...
class Measures:
    ...
    def __init__(self, data: pd.DataFrame, ccp_alpha=None, labels_col=None):
        ...
        self.dtc_pruned = None
        self.dtc_pruned_fit = False
        ...
        self.dist_matrix_gower = None
        self.indices_gower = None
        self.distances_gower = None
    ...
    def fit_tree_pruned(self):
        if self.ccp_alpha is None:
            parameters = {'ccp_alpha': np.linspace(0.001, 0.1, num=100)}
            dtc = tree.DecisionTreeClassifier(criterion='gini')
            clf = GridSearchCV(dtc, parameters)
            clf = clf.fit(self.X, self.y)
            self.ccp_alpha = clf.best_params_['ccp_alpha']

        self.dtc_pruned = tree.DecisionTreeClassifier(criterion='gini', ccp_alpha=self.ccp_alpha)
        self.dtc_pruned = self.dtc_pruned.fit(self.X, self.y)
        self.dtc_pruned_fit = True

    def gower_distance_matrix(self):
        # self.dist_matrix_gower = gower.gower_matrix(self.X.values.copy())
        self.dist_matrix_gower = gower_distance(self.X)
        delta = np.diag(-np.ones(self.dist_matrix_gower.shape[0]))
        self.indices_gower = np.argsort(self.dist_matrix_gower + delta, axis=1)
    ...
    def disjunct_class_percentage(self):
        ...
        if not self.dtc_pruned_fit:
            self.fit_tree_pruned()
    ...
    def class_likeliood_diff(self):
        print("clddddddddddddddddddddddddddddddddddddddd")
        n = len(self.y.unique())
        priors = np.ones((n,)) / n
        ...
        # print(prob[1, np.argwhere(nb.classes_ == labels[1])[0][0]]) = 1 1
        #print(np.argwhere(nb.classes_ == labels[1])[0][0]) 1 3
        #print(prob[1, :]) [0.00000000e+000 1.90020931e-032 1.90784001e-096 1.00000000e+000 3.95988256e-137]
        #print(np.delete(prob[1, :], np.argwhere(nb.classes_ == labels[1])[0][0])) [0.00000000e+000 1.90020931e-032 1.90784001e-096 3.95988256e-137]
        #print(np.max(np.delete(prob[1, :], np.argwhere(nb.classes_ == labels[1])[0][0]))) 1.9002093135223632e-32, depois que tira o 1 ele pega o maior

        #print(prob[1, np.argwhere(nb.classes_ == labels[1])[0][0]] - np.max(np.delete(prob[1, :], np.argwhere(nb.classes_ == labels[1])[0][0]))) = 1
    ...
    def borderline_points(self):
        ...
        if self.dist_matrix_gower is None:
            self.dist_matrix_gower = gower_distance(self.X)
            delta = np.diag(-np.ones(self.dist_matrix_gower.shape[0]))
            self.indices_gower = np.argsort(self.dist_matrix_gower + delta, axis=1)
            self.distances_gower = np.sort(self.dist_matrix_gower, axis=1)
